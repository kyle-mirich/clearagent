...
class PipelineSettings:
    ...
    synthetic_max_concurrency: int = 8
    ...
    try:
        ...
        _event(
            ...
            f"Generating {planned_cases} synthetic examples.",
            {
                ...
                "generation_mode": (
                    "deterministic" if pipeline_settings.deterministic_mode else "async"
                ),
                **(
                    {
                        "batch_count": (
                            planned_cases + SYNTHETIC_BATCH_SIZE - 1
                        )
                        // SYNTHETIC_BATCH_SIZE,
                        "max_concurrency": pipeline_settings.synthetic_max_concurrency,
                    }
                    if not pipeline_settings.deterministic_mode
                    else {}
                ),
...
async def _generate_dataset_live_async(
    *,
    profile: str,
    seed: int,
    task_spec: dict[str, Any],
    ...
    async def generate_batch(batch: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for attempt in range(3):
            generated = await _acomplete_structured(
    ...
    concurrency = max(1, min(settings.synthetic_max_concurrency, len(batches)))
    ...
    async def collect_batch(
        ...
        async with semaphore:
            try:
                ...
                return batch_index, [], _safe_pipeline_error(exc)

    pending = [
        asyncio.create_task(collect_batch(index, batch))
...
async def _acomplete_structured(
    model_uri: str,
    settings: PipelineSettings,
    messages: list[Message],
    response_model: type[StructuredModelT],
    *,
    ...
    token_limit = settings.gepa_max_tokens if max_tokens is None else max_tokens
    for attempt in range(3):
        text = await _aprovider_completion(
            model_uri,
            settings,
            request_messages,
            max_tokens=token_limit,
        ...
        try:
            ...
            request_messages.append(Message(role="user", content=correction))
    raise AssertionError("Structured completion loop exited unexpectedly.")
...
async def _aprovider_completion(
    model_uri: str,
    settings: PipelineSettings,
    messages: list[Message],
    *,
...
async def _aprovider_completion_with_provider(
    provider: Any,
    model_uri: str,
    settings: PipelineSettings,
    messages: list[Message],
    *,
    ...
    for empty_attempt in range(3):
        ...
        if response.output_text:
            return response.output_text
        if empty_attempt < 2:
            await asyncio.sleep(0.5 * (2**empty_attempt))
    raise RuntimeError(f"{model_uri} returned no text after three attempts.")
