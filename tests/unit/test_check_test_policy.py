...
def test_policy_rejects_alternate_static_analysis_configuration(tmp_path, filename):
    ...
    assert any(f"{filename}: alternate analysis configuration is forbidden" in item for item in violations)
...
def test_policy_rejects_extra_pytest_discovery_options(tmp_path):
    ...
    assert "pyproject.toml: pytest ini option 'python_files' is forbidden" in find_policy_violations(
        project
    )
