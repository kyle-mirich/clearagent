...
def test_authoritative_shell_gate_keeps_every_required_layer():
    ...
    commands = [
        line.strip().removeprefix('"${uv_run[@]}" ')
        for line in gate.splitlines()
        if line.strip().startswith('"${uv_run[@]}" ')
    ]

    assert gate.startswith("#!/usr/bin/env bash\nset -euo pipefail\n")
    assert "unset PYTEST_ADDOPTS PYTEST_PLUGINS\n" in gate
    assert "uv_run=(uv run --locked --no-sync)" in gate
    assert commands[0] == "python scripts/check_test_policy.py"
    ...
    assert "ruff check --config pyproject.toml --no-fix --no-respect-gitignore ." in commands
    assert "ruff format --check --config pyproject.toml ." in commands
    assert "python -m mypy --config-file pyproject.toml src" in commands
    assert "python scripts/check_docs_links.py" in commands
    assert commands[-1] == "python scripts/check_distribution.py"
...
def test_authoritative_shell_gate_stops_at_the_first_failed_layer(tmp_path):
    ...
    assert invocation_log.read_text(encoding="utf-8").splitlines() == [
        "run --locked --no-sync python scripts/check_test_policy.py"
    ]
