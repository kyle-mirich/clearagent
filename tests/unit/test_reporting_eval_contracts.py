...
def test_trace_checks_fail_when_trace_context_or_run_is_missing(tmp_path):
    ...
    orphaned_trace = run_checks(
        [{"max_turns": 1}, {"not_called_tool": "dangerous_tool"}],
        _result(run_id="orphaned"),
    )
    ...
    assert all(check.passed is False for check in orphaned_trace)
    assert all("trace data is unavailable" in check.message for check in orphaned_trace)
