...
def test_matrix_baseline_distinguishes_variants_of_the_same_case(tmp_path):
    ...
    assert comparison.regressions == ['case a [variant={"model":"openai:gpt","temperature":0.0}]']
    assert comparison.unchanged_passes == [
        'case a [variant={"model":"openai:gpt","temperature":0.2}]'
    ]
    with store.connect() as db:
        saved = db.execute("SELECT results_json FROM baselines WHERE name='matrix-v1'").fetchone()
...
@pytest.mark.parametrize(
    ("agent_name", "model", "expected_field"),
    [
        ("other-support", "openai:gpt", "agent_name"),
        ("support", "openai:other", "model"),
    ],
)
def test_compare_baseline_rejects_mismatched_agent_or_model(
    tmp_path, agent_name, model, expected_field
):
    store = SQLiteTraceStore(tmp_path / "traces.sqlite")
    baseline_run = store.start_eval_suite_run(
        suite_name="smoke", suite_type="output", agent_name="support", model="openai:gpt"
    ...
    mismatched_run = store.start_eval_suite_run(
        suite_name="smoke", suite_type="output", agent_name=agent_name, model=model
    )
