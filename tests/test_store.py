...
def test_concurrent_sqlite_version_allocation_is_unique_and_project_monotonic(tmp_path):
    store = Store(f"sqlite:///{tmp_path / 'engine.sqlite'}")
...
def test_dataset_generation_degrades_with_a_clear_error_when_every_batch_fails(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("clearagent.builds.pipeline._acomplete_structured", failing_complete)
    skipped = []
...
def test_dataset_generation_launches_all_quick_batches_concurrently(monkeypatch):
    planning = plan_task("Build an agent that drafts polite customer refund explanations.")
    assert planning.task_spec is not None
    task_spec = planning.task_spec.model_dump()
    expected_layout = pipeline_module.generate_synthetic_examples(
        profile="quick", seed=3, task_spec=task_spec, n=30
    )
    active = 0
    max_active = 0
    ...
    async def fake_complete(_model, _settings, messages, _response_model, **_kwargs):
        ...
        active -= 1
        return pipeline_module.GeneratedExampleBatch(
            examples=[
                pipeline_module.GeneratedExample(
                    id=item["id"],
                    input={"message": f"input for {item['id']}"},
                    expected={"answer": f"answer for {item['id']}"},
                    reference_notes=f"Reward the expected behavior for {item['id']}.",
    ...
    monkeypatch.setattr(pipeline_module, "_acomplete_structured", fake_complete)
    dataset = asyncio.run(
        pipeline_module._generate_dataset_live_async(
            profile="quick",
            seed=3,
            task_spec=task_spec,
            n=30,
            settings=PipelineSettings(
                ...
                synthetic_max_concurrency=8,
            ),
        )
    )

    assert max_active == 5
    assert [item["id"] for item in dataset["examples"]] == [
        item["id"] for item in expected_layout["examples"]
    ]
