...
def test_local_url_provider_routes_to_the_parsed_base_url(monkeypatch):
    ...
    assert getattr(provider.chat_model, "openai_api_base") == "http://localhost:8000/v1"
    assert request.body["model"] == "llama3.1"


def test_ollama_provider_defaults_to_the_local_inference_endpoint(monkeypatch):
    monkeypatch.setenv("OPENAI_API_KEY", "should-not-be-used")

    provider = provider_for_model("ollama:llama3.1")

    assert getattr(provider.chat_model, "openai_api_base") == "http://localhost:11434/v1"


def test_local_provider_defaults_to_the_local_compatible_endpoint(monkeypatch):
    monkeypatch.setenv("OPENAI_API_KEY", "should-not-be-used")
